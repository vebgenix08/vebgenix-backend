...
if (!stage || !['dev', 'prod'].includes(stage)) {
  throw new Error('Usage: npx cdk deploy -c env=dev  OR  npx cdk deploy -c env=prod');
}
...
const networkStack = new NetworkStack(app, `VebgenixNetwork-${config.stage}`, { env, config });

// 2. Auth: Cognito User Pool
const authStack = new AuthStack(app, `VebgenixAuth-${config.stage}`, { env, config });

// 3. Storage: Private S3 Bucket
const storageStack = new StorageStack(app, `VebgenixStorage-${config.stage}`, { env, config });
