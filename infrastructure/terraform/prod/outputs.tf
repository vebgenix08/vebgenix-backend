output "atlas_cluster_connection_string" {
  ...
  value       = mongodbatlas_cluster.prod.connection_strings[0].standard_srv
  sensitive   = true
}

output "atlas_project_id" {
  description = "MongoDB Atlas project ID"
  value       = mongodbatlas_project.prod.id
}
...
output "razorpay_secret_name" {
  description = "Secrets Manager secret name for Razorpay live keys (populate manually)"
  value       = aws_secretsmanager_secret.razorpay.name
}
