...
const ALLOWED_TENANT_ROLES = ['ACCOUNTANT', 'TEACHER', 'STUDENT', 'STAFF', 'PARENT'];
// Fallback constant if env var is missing, usually for local dev
const DEFAULT_APP_BASE_URL = process.env.APP_BASE_URL || "http://localhost:5173";
...
async function getOrCreateAuthUser(email: string, fullName: string) {
  ...
  // B2: Store only full_name in metadata, NO ROLE
...
async function isPlatformUser(email: string): Promise<boolean> {
    if (email.toLowerCase() === PLATFORM_SUPER_ADMIN_EMAIL.toLowerCase()) return true;
...
export class UserController {
  ...
  static async getUsers(req: Request, res: Response) {
    try {
      ...
      const { query, role, status, campus_id, page = 1, limit = 20 } = req.query;
      ...
      const users = rows.map((u) => ({
        ...
        campusAccess: u.campusAccess.map((ca) => ({ campus: { id: ca.campus.id, name: ca.campus.name, campusType: ca.campus.campusType } })),
        employee: u.employee
          ? {
              ...
              joined_on: u.employee.joinedOn?.toISOString?.()?.slice(0, 10) ?? null,
  ...
  static async getUser(req: Request, res: Response) {
    try {
      ...
      const out = {
        ...
        campusAccess: user.campusAccess.map((ca) => ({ campus: { id: ca.campus.id, name: ca.campus.name, campusType: ca.campus.campusType } })),
        employee: user.employee
          ? {
              ...
              joined_on: user.employee.joinedOn?.toISOString?.()?.slice(0, 10) ?? null,
  ...
  static async createUser(req: Request, res: Response) {
    try {
      const tenantId = (req as any).tenant?.tenantId;
      const tenantSubdomain = (req as any).tenant?.subdomain;
      if (!tenantId) throw new Error("Tenant context missing");
      ...
      if (!ALLOWED_TENANT_ROLES.includes(roleUpper)) {
          ...
          if (['ADMIN', 'SUPER_ADMIN'].includes(roleUpper)) {
             return res.status(403).json({
                error: { code: "ADMIN_ROLE_FORBIDDEN", message: "Only Super Admin creates tenant admins via platform routes" }
             });
          }
          return res.status(400).json({
             error: { code: "INVALID_ROLE", message: `Role ${roleUpper} is not allowed.` }
          });
      }
      ...
      if (await isPlatformUser(normalizedEmail)) {
        return res.status(409).json({
            error: { code: "PLATFORM_EMAIL_FORBIDDEN", message: "Platform users cannot be added as tenant users" }
        });
      }
      ...
      if (Array.isArray(campus_ids) && campus_ids.length > 0) {
          ...
          if (validCount !== campus_ids.length) {
              return res.status(404).json({
                  error: { code: "CAMPUS_NOT_FOUND", message: "One or more provided campuses do not belong to this tenant." }
              });
          }
      }
      ...
      if (['TEACHER', 'STUDENT', 'STAFF'].includes(roleUpper)) {
          if (all_campuses_access) {
              return res.status(400).json({
                  error: { code: "CAMPUS_ACCESS_REQUIRED", message: `${roleUpper} cannot have all-campus access. Specific campus assignment required.` }
              });
          }
          if (!campus_ids || !Array.isArray(campus_ids) || campus_ids.length === 0) {
              return res.status(400).json({
                  error: { code: "CAMPUS_ACCESS_REQUIRED", message: `${roleUpper} must be assigned to at least one campus.` }
              });
          }
      }

      // Validate Campus Mix for Restricted Roles (Cross-Type)
      if (['TEACHER', 'STAFF'].includes(roleUpper) && campus_ids && campus_ids.length > 1) {
          ...
          if (types.has('SCHOOL') && types.has('PU')) {
              return res.status(409).json({
                  error: { 
                      ...
                      message: 'Teachers and Staff cannot be assigned to both School and College campuses.' 
                  }
              });
          }
      }

      // 1. One-Tenant-Per-Email Enformcement
      ...
      if (existingProfile && existingProfile.tenantId !== tenantId) {
        return res.status(409).json({
          error: { code: "EMAIL_IN_USE", message: "Email already registered in another tenant" },
        });
      }

      // B2: Get or Create Auth User (No Role in Metadata)
      const { userId, alreadyExisted } = await getOrCreateAuthUser(normalizedEmail, full_name);
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (employee) {
          if (employee.employee_code) {
             ...
             if (existingEmp && existingEmp.authUserId !== userId) {
               throw new Error(`Employee code '${employee.employee_code}' already in use`);
             }
          }
          ...
          await tx.employee.upsert({
            ...
            create: {
              ...
              joinedOn: employee.joined_on ? new Date(employee.joined_on) : null,
            },
            update: {
              employeeCode: employee.employee_code,
              phone: employee.phone,
      ...
      if (sendInvite) {
        ...
        const { data: linkData, error: linkError } = await supabase.auth.admin.generateLink({
          type: "recovery", // Changed from "invite" to "recovery"
          email: normalizedEmail,
        ...
        if (!linkError && linkData.properties?.action_link) {
          await EmailService.sendMail(
            ...
            `<p>You have been invited to join <strong>${tenantSubdomain}</strong> on ERP.</p>
  ...
  static async updateUser(req: Request, res: Response) {
    try {
      ...
      const { id } = req.params;
      const { full_name, role, all_campuses_access, campus_ids, employee, is_active } = req.body;
      ...
      if (await isPlatformUser(userProfile.email)) {
         return res.status(409).json({
            error: { code: "PLATFORM_EMAIL_FORBIDDEN", message: "Platform users cannot be modified via tenant routes" }
         });
      }
      ...
      if (role) {
          ...
          if (!ALLOWED_TENANT_ROLES.includes(roleUpper!)) {
              if (['ADMIN', 'SUPER_ADMIN'].includes(roleUpper!)) {
                  return res.status(403).json({
                      error: { code: "ADMIN_ROLE_FORBIDDEN", message: "Cannot promote to ADMIN via this endpoint" }
                  });
              }
              return res.status(400).json({
                  error: { code: "INVALID_ROLE", message: `Role ${roleUpper} is not allowed.` }
      ...
      if (id === requesterId) {
          if (is_active === false) {
             return res.status(409).json({ error: { code: "SELF_LOCKOUT_FORBIDDEN", message: "You cannot deactivate your own account." } });
          }
          if (all_campuses_access === false && (!campus_ids || campus_ids.length === 0)) {
             return res.status(409).json({ error: { code: "SELF_LOCKOUT_FORBIDDEN", message: "You cannot remove all campus access from yourself." } });
          }
      }

      // 1. Guard: First Admin Role Lock
      const currentTenant = await prisma.tenant.findUnique({ where: { id: tenantId } });
      if (currentTenant?.firstAdminId === id) {
        if (roleUpper && roleUpper !== 'ADMIN') { // Technically roleUpper won't be ADMIN here due to B1 check above?
           // Wait, if first admin IS admin, and we try to change to TEACHER, roleUpper is TEACHER.
           // So this check is valid: preventing demotion.
           return res.status(409).json({
             error: { code: "FIRST_ADMIN_ROLE_LOCKED", message: "Cannot change role of the primary tenant admin." }
           });
        }
        if (is_active === false) {
           return res.status(409).json({
             error: { code: "FIRST_ADMIN_LOCKED", message: "Cannot deactivate the primary tenant admin." }
           });
        }
        if (all_campuses_access === false) {
             return res.status(409).json({
                 error: { code: "FIRST_ADMIN_ACCESS_LOCKED", message: "Cannot remove all-campus access from the primary tenant admin." }
             });
        }
      }
      ...
      const effectiveAllAccess = all_campuses_access !== undefined ? all_campuses_access : userProfile.allCampusesAccess;
      ...
      if (['TEACHER', 'STUDENT', 'STAFF'].includes(effectiveRole)) {
          ...
          if (all_campuses_access !== undefined || campus_ids !== undefined) {
               if (effectiveAllAccess) {
                   return res.status(400).json({
                       error: { code: "CAMPUS_ACCESS_REQUIRED", message: `${effectiveRole} cannot have all-campus access.` }
                   });
               }
               ...
               if (campus_ids !== undefined) {
                   if (!Array.isArray(campus_ids) || campus_ids.length === 0) {
                       return res.status(400).json({
                           error: { code: "CAMPUS_ACCESS_REQUIRED", message: `${effectiveRole} must be assigned to at least one campus.` }
                       });
                   }
               } else if (all_campuses_access === false && userProfile.allCampusesAccess === true) {
                   ...
                   return res.status(400).json({
                       error: { code: "CAMPUS_ACCESS_REQUIRED", message: `Please select campuses for ${effectiveRole}.` }
                   });
               }
               ...
               if (userProfile.allCampusesAccess) {
                   return res.status(400).json({
                       error: { code: "CAMPUS_ACCESS_REQUIRED", message: `Role ${effectiveRole} requires specific campus assignment. Please update access.` }
                   });
               }
               // If existing user has specific access, check if it has any campuses.
               const existingCount = await prisma.userCampusAccess.count({ where: { userId: id } });
               if (existingCount === 0) {
                   return res.status(400).json({
                       error: { code: "CAMPUS_ACCESS_REQUIRED", message: `Role ${effectiveRole} requires at least one campus.` }
                   });
               }
          }
      }
      ...
      // B2: Update only full_name in Supabase. Never role.
      if (full_name) {
      ...
      await prisma.$transaction(async (tx) => {
         ...
         if (all_campuses_access !== undefined) updateData.allCampusesAccess = !!all_campuses_access;
         ...
         if (all_campuses_access === true) {
            await tx.userCampusAccess.deleteMany({ where: { userId: id } });
      ...
      const updated = await prisma.profile.findUnique({
        where: { id },
        include: { employee: true, campusAccess: { include: { campus: true } } },
      });
      if (!updated) return res.status(500).json({ error: { code: "INTERNAL_ERROR", message: "Failed to load updated user" } });
      const out = {
        id: updated.id,
        email: updated.email,
        ...
        campusAccess: updated.campusAccess.map((ca) => ({ campus: { id: ca.campus.id, name: ca.campus.name, campusType: ca.campus.campusType } })),
        employee: updated.employee
          ? {
              ...
              joined_on: updated.employee.joinedOn?.toISOString?.()?.slice(0, 10) ?? null,
  ...
  static async resendInvite(req: Request, res: Response) {
    try {
      ...
      if (await isPlatformUser(userProfile.email)) {
         return res.status(409).json({
            error: { code: "PLATFORM_EMAIL_FORBIDDEN", message: "Cannot send invite to platform users" }
         });
      }
      ...
      // Remove ?next=/change-password&mode=recovery as Callback.tsx handles it directly
      const redirectUrl = `${origin}/auth/callback`;
      ...
      return res.status(500).json({ error: { message: "Failed to resend invite" } });
    }
  }
  ...
  static async resetPassword(req: Request, res: Response) {
    try {
      ...
      if (await isPlatformUser(userProfile.email)) {
         return res.status(409).json({
            error: { code: "PLATFORM_EMAIL_FORBIDDEN", message: "Cannot reset password for platform users via tenant portal" }
         });
      }
      ...
      return res.status(500).json({ error: { message: "Failed to send reset link" } });
    }
  }
}
