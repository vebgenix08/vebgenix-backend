...
interface PromoteStudentsInput {
  ...
  sectionStrategy:
    ...
    | 'TRANSPORT_ROUTE'
    | 'EXCEL_IMPORT';
  sectionAssignments?:       Array<{ studentId: string; sectionId: string }>;
  targetSectionIds?:         string[];
  maxStudentsPerSection?:    number;
  rankByExamId?:             string;
  subjectGroupSectionMap?:   Array<{ subjectGroupId: string; sectionId: string }>;
  transportRouteSectionMap?: Array<{ transportRouteId: string; sectionId: string }>;
  eligibilityMode?: 'USE_ENROLLMENT_ELIGIBILITY' | 'IGNORE_RESULTS';
...
async function createPromotionInvoiceFromStructure(
  ...
  // FeeCategory model removed — use default prefixes
...
// ── PromoteStudents use-case (inlined) ───────────────────────────────────────

async function promoteStudents(ctx: AuthContext, input: PromoteStudentsInput) {
...
export async function handlePromotions(
  ...
  switch (operation) {
    ...
    case 'POST:/api/admin/academics/promotions': {
      const input = ((args.input as Record<string, unknown>) ?? args) as unknown as PromoteStudentsInput;
      const result = await promoteStudents(ctx, input);
      return toGql(result);
    }
