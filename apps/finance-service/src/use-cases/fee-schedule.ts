...
import { toGql, validatePercentTotal } from '../finance-utils';

export async function handleFeeSchedule(
  operation: string,
  ...
  switch (operation) {
    ...
    case 'POST:/api/admin/finance/fee-schedules': {
      ...
      const { name, academicYearId, collectionType, minimumAmount, minimumPercentage, slots } = payload;
      if (!name || !academicYearId) {
        throw new AppError('BAD_REQUEST', 'name and academicYearId are required');
      }
      if (collectionType === 'PARTIAL_WITH_MINIMUM_AMOUNT' && minimumAmount != null) {
        if ((minimumAmount as number) <= 0) {
          throw new AppError('BAD_REQUEST', 'minimumAmount must be > 0 when collectionType is PARTIAL_WITH_MINIMUM_AMOUNT');
        }
      }
      if (collectionType === 'PARTIAL_WITH_MINIMUM_PERCENTAGE' && minimumPercentage != null) {
      ...
      if (Array.isArray(slots) && slots.length > 0) {
        validatePercentTotal(slots as { percentOfTotal?: number }[]);
      }
    ...
    case 'PATCH:/api/admin/finance/fee-schedules/:id': {
      ...
      if (Array.isArray(update.slots) && update.slots.length > 0) {
        validatePercentTotal(update.slots as { percentOfTotal?: number }[]);
      }
