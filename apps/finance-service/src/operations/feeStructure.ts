...
// ── Inline helpers used by feeStructureMapping ────────────────────────────────
...
export async function assignFeeStructureToStudent(
  ...
  const { StudentFeeOrderService } = await import('./studentOrder.js');
  ...
  // Fall back to Student document fields if no enrollment exists
